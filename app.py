...
# 2. 서버 공용 메모리 설정 (Streamlit Cloud에서는 완벽하게 100% 동기화됨)
@st.cache_resource
...
# 4. 메인 게임 로직
    ...
    if st.session_state.user_role == "admin":
        ...
        if not is_game_over:
            if st.button(f"🚀 다음 시대로 점프! ({year_list[turn_idx+1] if turn_idx < len(year_list)-1 else '종료'}년)", type="primary", use_container_width=True):
        ...
        # ⭐ 초기화 버그 완벽 해결 로직 ⭐
        st.markdown("### 💹 가격 조작 패널")
        st.caption("이제 입력 도중에 숫자가 절대로 초기화되지 않습니다!")
    ...
    # ----------------------------------------
        ...
        if is_game_over:
            ...
            with st.container(border=True):
                st.write(f"🪙 **안드로 코인** (현재가: {current_prices[special_coin]:,}원)")
                owned_coin = my_data["portfolio"][special_coin]
                st.caption(f"보유량: {owned_coin}개")
                c1, c2 = st.columns(2)
                with c1:
                    b_amt = st.number_input("코인 매수 수량", min_value=0, step=1, key="b_coin")
                    st.caption(f"예상: -{b_amt * current_prices[special_coin]:,}원")
                    if st.button("매수", key="btn_b_coin", use_container_width=True):
                        if b_amt * current_prices[special_coin] <= my_data["cash"]:
                            ...
                            st.error("현금이 부족합니다!")
                with c2:
                    s_amt = st.number_input("코인 매도 수량", min_value=0, step=1, key="s_coin")
                    st.caption(f"예상: +{s_amt * current_prices[special_coin]:,}원")
                    if st.button("매도", key="btn_s_coin", use_container_width=True):
                        if s_amt <= owned_coin:
            ...
            for i, stock in enumerate(constellations):
                with display_cols[i % 2]:
                    with st.container(border=True):
                        st.write(f"**{stock}** (현재가: {current_prices[stock]:,}원)")
                        owned = my_data["portfolio"][stock]
                        st.caption(f"보유량: {owned}주")
                        c1, c2 = st.columns(2)
                        with c1:
                            b_amt = st.number_input("매수", min_value=0, step=1, key=f"b_n_{stock}")
                            st.caption(f"예상: -{b_amt * current_prices[stock]:,}원")
                            if st.button("매수", key=f"b_b_{stock}", use_container_width=True):
                                if b_amt * current_prices[stock] <= my_data["cash"]:
                        ...
                        with c2:
                            ...
                            if st.button("매도", key=f"s_b_{stock}", use_container_width=True):
                                if s_amt <= owned:
